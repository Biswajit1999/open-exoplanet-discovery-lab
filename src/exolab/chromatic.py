...
from .rv import fit_shared_sinusoid as fit_release_02_sinusoid
...
def simultaneity_counts(
    ...
    *,
    windows_hours: Iterable[float] | None = None,
) -> dict[str, int] | dict[float, int]:
    if windows_hours is not None:
        return {
            float(hours): len(
                matched_epoch_pairs(optical_time, nir_time, float(hours) / 24.0)
            )
...
def compare_bands(
    ...
    *,
    ...
    """Release-0.2 comparison with each series referenced to its own median."""
    band_a = fit_release_02_sinusoid(time_a, rv_a, err_a, period=period)
    band_b = fit_release_02_sinusoid(time_b, rv_b, err_b, period=period)
    ratio = (
        np.nan
        if band_a.semi_amplitude == 0
        else band_b.semi_amplitude / band_a.semi_amplitude
    )
    return CoherenceResult(
        period=float(period),
        k_a=band_a.semi_amplitude,
        k_b=band_b.semi_amplitude,
        amplitude_ratio=float(ratio),
        phase_a=band_a.phase,
        phase_b=band_b.phase,
        phase_delta=wrapped_phase_difference(band_b.phase, band_a.phase),
    )
