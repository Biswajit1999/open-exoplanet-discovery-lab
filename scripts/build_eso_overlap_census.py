...
def query_harps(
    ...
    for attempt in range(max(1, int(retries))):
        try:
            ...
            return row, client.query(query, maxrec=100000), ""
        except Exception as exc:
            last_error = repr(exc)
            if attempt + 1 < retries:
                time.sleep(min(8.0, 1.5 * (attempt + 1)))
    return row, pd.DataFrame(), last_error
...
def main() -> int:
    ...
    rights_clause = (
        ...
        if any(column.lower() == "data_rights" for column in available_columns)
        else ""
    )
    count_query = (
        "SELECT COUNT(*) AS n FROM ivoa.ObsCore "
        "WHERE instrument_name='NIRPS'" + rights_clause
    )
    expected_nirps_products = int(client.query(count_query).iloc[0, 0])
    nirps = client.query(nirps_query, maxrec=max(100000, expected_nirps_products + 1))
    if nirps.empty:
        raise RuntimeError(
    ...
    if len(nirps) != expected_nirps_products:
        raise RuntimeError(
            "ESO ObsCore inventory is truncated: "
            f"count query returned {expected_nirps_products}, inventory returned {len(nirps)}"
        )
    ...
    metadata = {
        ...
        "nirps_products_count_query": expected_nirps_products,
